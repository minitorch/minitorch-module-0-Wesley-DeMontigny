...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert_close(1.0 - sig, sigmoid(-1.0 * a))
    assert sigmoid(0) == 0.5
    assert sigmoid(0.0) < sigmoid(0.25) and sigmoid(0.25) < sigmoid(0.5) and sigmoid(0.5) < sigmoid(0.75) and sigmoid(0.75) < sigmoid(1.0) # Kinda hacky 
...
def test_other() -> None:
    ...
    a = 2
    b = 3
    assert eq(a, b) != (lt(a, b) or lt(b, a))
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    for i in range(len(ls1)):
        sum_test += ls1[i] + ls2[i]
    assert_close(sum(ls1) + sum(ls2), sum_test) 
